...
//--></script></head><body width="100%"><div id="loader" ><?php echo L('load_page');?></div><div id="result" class="result none"></div><div class="mainbox"><?php if(empty($_GET['isajax'])): ?><div id="nav" class="mainnav_title"><div id="lang"><?php if(APP_LANG): parse_str($_SERVER['QUERY_STRING'],$urlarr); unset($urlarr[l]); $url='index.php?'.http_build_query($urlarr); if(is_array($Lang)): $i = 0; $__LIST__ = $Lang;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$langvo): $mod = ($i % 2 );++$i;?><a href="<?php echo ($url); ?>&l=<?php echo ($langvo["mark"]); ?>" <?php if($langvo[mark]==$_SESSION['YP_lang']): ?>class="on"<?php endif; ?>><?php echo ($langvo["name"]); ?></a><?php endforeach; endif; else: echo "" ;endif; endif; ?></div><ul><a href="<?php echo U($nav[bnav][model].'/'.$nav[bnav][action],$nav[bnav][data]);?>"><?php echo ($nav["bnav"]["name"]); ?></a>|
	...
	</script><?php endif; ?><form name="myform" id="myform" action="<?php echo U($module_name.'/'.$action_name);?>" method="post"><table cellpadding=0 cellspacing=0 id="param" class="table_form" width="100%"><tr><td width="120"><?php echo L(param_name);?></td><td ><?php echo L(param_value);?></td></tr><?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr class="asf"><td width="120"><?php echo ($key); ?></td><td class="da"><input type="text" id="<?php echo ($key); ?>" name="<?php echo ($key); ?>" value="<?php echo (htmlspecialchars($vo)); ?>" class="input-text" /><a href="javascript:void(0);" onclick="deletethis(this);"><?php echo L(delete);?></a></td></tr><?php endforeach; endif; else: echo "" ;endif; ?></table><div id="bootline"></div><div id="btnbox" class="btn"><INPUT TYPE="submit"  name="dosubmit" value="<?php echo L('dosubmit');?>" class="button" ><input type="button" value="<?php echo L(add_tpl_param);?>" class="button" onclick="addparam();" /><input TYPE="reset"  value="<?php echo L('cancel');?>" class="button"></div></form></div><script>
	...
	var dialog = art.dialog({
		...
		ok: function () {
			...
			if (key.value  !== '' && value.value  !== '' ) {
				...
				topWin.$('#param').append('<tr><td width="120">'+key.value+'</td><td><input type="text"   name="'+key.value+'" value="'+value.value+'" class="input-text" /><a href="javascript:void(0);" onclick="deletethis(this);"><?php echo L(delete);?></a></td></tr>');
				art.dialog({
					time: 1,
