...
//--></script></head><body width="100%"><div id="loader" ><?php echo L('load_page');?></div><div id="result" class="result none"></div><div class="mainbox"><?php if(empty($_GET['isajax'])): ?><div id="nav" class="mainnav_title"><div id="lang"><?php if(APP_LANG): parse_str($_SERVER['QUERY_STRING'],$urlarr); unset($urlarr[l]); $url='index.php?'.http_build_query($urlarr); if(is_array($Lang)): $i = 0; $__LIST__ = $Lang;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$langvo): $mod = ($i % 2 );++$i;?><a href="<?php echo ($url); ?>&l=<?php echo ($langvo["mark"]); ?>" <?php if($langvo[mark]==$_SESSION['YP_lang']): ?>class="on"<?php endif; ?>><?php echo ($langvo["name"]); ?></a><?php endforeach; endif; else: echo "" ;endif; endif; ?></div><ul><a href="<?php echo U($nav[bnav][model].'/'.$nav[bnav][action],$nav[bnav][data]);?>"><?php echo ($nav["bnav"]["name"]); ?></a>|
	...
	</script><?php endif; ?><form name="myform" action="<?php echo U('Module/listorder');?>" method="post"><div class="table-list"><table width="100%" cellspacing="0"><thead><tr><th width="40">ID</th><th width="90" align="left" ><?php echo L('module_name');?></th><th width="90" align="left" ><?php echo L('module_table');?></th><th width="300" align="left" ><?php echo L('module_description');?></th><th width="240"><?php echo L('manage');?></th></tr></thead><tbody><?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr><td align="center"><?php echo ($vo['id']); ?></td><td><?php echo ($vo['title']); ?></td><td><?php echo ($vo['name']); ?></td><td><?php echo ($vo['description']); ?></td><td align="center"><a href="<?php echo U('Field/index',array(moduleid=>$vo['id']));?>"><?php echo L('module_field');?></a> | 
			<a href="<?php echo U('Module/edit',array(id=>$vo['id']));?>"><?php echo L('edit');?></a>		| 
			<?php if($vo[status]==1): ?><a href="<?php echo U('Module/status',array(id=>$vo['id'],status=>0));?>"><font color="green"><?php echo L('enable');?></font></a><?php else: ?><a href="<?php echo U('Module/status',array(id=>$vo['id'],status=>1));?>"><font color="red"><?php echo L('disable');?></font></a><?php endif; ?>
		 | <?php if($vo['issystem']): ?><font color="#CFCFCF"><?php echo L('delete');?></font><?php else: ?><a href="<?php echo U('Module/delete',array(id=>$vo['id']));?>"  onclick="return confirm('<?php echo L('module_real_uninstall');?>')"><?php echo L('delete');?></a><?php endif; ?></td></tr><?php endforeach; endif; else: echo "" ;endif; ?></tbody></table></div></form></div></body></html>